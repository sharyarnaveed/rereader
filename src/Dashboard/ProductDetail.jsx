...
const ProductDetail = () => {
  ...
  const tabs = [
    ...
    },
    {
      id: "unsold",
      label: "Unsold",
      count: products.filter((p) => p.status === "unsold").length,
    },
  ];
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      const responce = await api.delete(`/api/user/deleteproduct/${productId}`);
            ...
            <div className="text-center py-12">
              ...
              {activeTab === "all" && (
                <button
                  ...
                  style={{ fontFamily: "K2D, sans-serif" }}
                  onClick={() =>
                    (window.location.href = "/userdashboard/upload")
                  }
                >
